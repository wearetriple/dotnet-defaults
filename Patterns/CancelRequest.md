...
- Controller / Function: Add `CancellationToken token` as parameter to accept the 
cancellation token in the controller method.
- Pass the token to all subsequent async calls. 
- Optional: Catch the `TaskCanceledException` in a middleware to log the cancellation.
